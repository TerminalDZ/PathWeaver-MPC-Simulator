...
import MPCController from "./autonomy/control/MPCController.js";
...
export default class Simulator {
  constructor(domElement) {
    ...
    this.mpcModeButton = document.getElementById('mode-mpc');
    this.mpcModeButton.addEventListener('click', this.enableMPCMode.bind(this));
  ...
  enableManualMode() {
    ...
    this.mpcModeButton.classList.add('is-outlined', 'is-inverted');
    this.mpcModeButton.classList.remove('is-selected', 'is-link');
  ...
  enableMPCMode() {
    this.mpcModeButton.classList.remove('is-outlined', 'is-inverted');
    this.mpcModeButton.classList.add('is-selected', 'is-link');
    this.manualModeButton.classList.add('is-outlined');
    this.manualModeButton.classList.remove('is-selected');
    this.autonomousModeButton.classList.add('is-outlined', 'is-inverted');
    this.autonomousModeButton.classList.remove('is-selected', 'is-link');

    this.carControllerMode = 'mpc';
  }
  ...
  receivePlannedPath(event) {
    ...
    if (this.mpcCarController) {
      this.mpcCarController.replacePath(followPath);
    } else {
      this.mpcCarController = new MPCController(followPath);
    }
  ...
  step(timestamp) {
    ...
    if (!this.editor.enabled && !this.paused) {
      ...
      let mpcControls = { steer: 0, brake: 0, gas: 0};
      if (this.autonomousCarController)
      ...
      if (this.carControllerMode == 'autonomous') {
        controls = autonomousControls;
      } else if (this.carControllerMode == 'mpc' && this.mpcCarController) {
        mpcControls = this.mpcCarController.control(this.car.pose, this.car.wheelAngle, this.car.velocity, dt);
        controls = mpcControls;
      } else {
      ...
      if (this.carControllerMode == 'autonomous') {
        this.dashboard.update(autonomousControls, carVelocity, this.carStation, latitude, this.simulatedTime, this.averagePlanTime.average);
      } else if (this.carControllerMode == 'mpc') {
        this.dashboard.update(mpcControls, carVelocity, this.carStation, latitude, this.simulatedTime, this.averagePlanTime.average);
      } else {
        this.dashboard.update(manualControls, carVelocity, this.carStation, latitude, this.simulatedTime, this.averagePlanTime.average);
      }
    }
